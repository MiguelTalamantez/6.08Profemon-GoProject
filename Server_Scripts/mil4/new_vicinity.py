...
###################################
# Get locations of nearby players #
###################################
...
def request_handler(request):
    ...
    sixty_sec_ago = datetime.datetime.now()- datetime.timedelta(seconds = 60) 
    
    close_users = c.execute('''SELECT DISTINCT user, lat, lon FROM location_table WHERE timing > ? AND MIT_loc = ?''',(sixty_sec_ago, mit_loc)).fetchall()
    conn.commit()
    conn.close()
    ...
    # # return closest_user
    # user_info = c.execute('''SELECT * from location_table WHERE user = (?) ORDER BY timing DESC;''', (user, )).fetchone()
    # # return user_info
    # other_info = c.execute('''SELECT * from location_table WHERE user = (?) ORDER BY timing DESC;''', (closest_user, )).fetchone()
    # conn.commit()
    # conn.close()

    # # both need to already exist in the table in database and be between 30 seconds of each other
    # if user_info is None or other_info is None:
    #     return "person not in database"

    # user_loc = (user_info[1], user_info[2])
    # other_loc = (other_info[1], other_info[2])
    # distance_km = distance(user_loc, other_loc)
    # distance_m = distance_km*1000

    # returned = closest_user
    # if distance_m > 10:
    #     returned += '\r'
    # return returned 