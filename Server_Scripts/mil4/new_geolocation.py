# -*- coding: utf-8 -*-
"""
Created on Wed Apr 13 01:17:06 2022
...
def min_distance(point, locations):
    best_distance = -1
    best_location = ""
    for  name in locations:
        test_distance = distance(point, locations[name]['center'])
        if best_distance == -1:
...
def request_handler(request):
    ...
    point = (lat, lon)
    place = get_area(point, locations)
    
    # conn = sqlite3.connect('/var/jail/home/team3/realtime_loc.db')
    # c = conn.cursor()
    # c.execute('''CREATE TABLE IF NOT EXISTS geolocation (user text, lat real, lon real, place text, timing timestamp);''')
    # c.execute('''INSERT OR REPLACE INTO geolocation (user, lat, lon, place, timing) VALUES( (SELECT user from geolocation WHERE user = ?), ?,?,?,?);''', (user, lat, lon, place, datetime.datetime.now()))


    if place == "Off Campus":
        best_values = min_distance(point, locations)
        # conn.commit()
        # conn.close()
        return "Nearest hotspot is " + str(best_values[1]) + ". Distance is " + str(best_values[0])
    ...
    return "You are in " + str(place) + "."
    
#--------------------debugging----------------------

# #different long and lat
# p1 = (42.359763,-71.092070)`
# p2 = (42.357943,-71.097773)

# #same long, different lat, debugs dlat
#p1 = (42.359763,-71.092070)
# # p2 = (42.357943,-71.092070)

# #same lat, different long, debugs dlon
#p1 = (42.359763,-71.092070)
# # p2 = (42.359763,-71.097773)

#print(min_distance(p1, locations))
